...
def load_test_spatial_coordinates(nb_points, coordinate_types):
    return [coordinate_class.from_nb_points(nb_points=nb_points)
            for coordinate_class in coordinate_types]


def load_test_1D_and_2D_spatial_coordinates(nb_points) -> List[
    AbstractSpatialCoordinates]:
    return load_test_spatial_coordinates(nb_points, TEST_1D_AND_2D_SPATIAL_COORDINATES)

def load_test_temporal_coordinates(nb_steps) -> List[AbstractTemporalCoordinates]:
...
def load_test_spatiotemporal_coordinates(nb_points, nb_steps):
