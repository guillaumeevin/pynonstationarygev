...
class ModelAsTruthExperiment(AbstractExperiment):
    ...
    def run_all_experiments(self):
        return np.nansum([self.run_one_experiment(gcm_rcm_couple_as_pseudo_truth=c) for c in
                          self.gcm_rcm_couples_sampled_for_experiment], axis=0)
    ...
    def load_studies_obs_for_train(self, gcm_rcm_couple_as_pseudo_truth) -> AltitudesStudies:
        """For gcm_rcm_couple_set_as_truth, load the data from 2020 to 2100"""
        return self.load_altitude_studies(gcm_rcm_couple_as_pseudo_truth, 1959, 2019)
